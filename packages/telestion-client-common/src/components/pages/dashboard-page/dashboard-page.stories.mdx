...
<Canvas>
	...
	>
		{args => {
			...
			const { eventBus, openEventBus } = useEventBus.getState();
			if (!eventBus) openEventBus('http://localhost:54321/nirwana');
...
<Canvas>
	...
	>
		{args => {
			useAuth.setState({ auth: { username: 'alice' } });
			const { eventBus, closeEventBus } = useEventBus.getState();
			if (eventBus) closeEventBus();
			useUserConfig.setState({
				userConfig: {
					alice: {
						dashboards: [args.dashboard]
